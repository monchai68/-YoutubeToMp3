...
class YouTubeToMP3Converter:
    ...
    def convert_to_mp3(self, input_file, output_file, quality):
        """Convert audio file to MP3 using FFmpeg"""
        try:
            self.update_status("Converting to MP3...")
            self.log("Converting to MP3 format...")
            
            # Set audio bitrate based on quality
            if quality == "best":
                bitrate = "320k"
            elif quality in ["320", "256", "192", "128"]:
                bitrate = f"{quality}k"
            else:
                bitrate = "320k"
            ...
            if result.returncode == 0:
                self.log(f"✓ Successfully converted to MP3: {os.path.basename(output_file)}")
                return True
            else:
    ...
    def progress_hook(self, d):
        ...
        if d['status'] == 'downloading':
            try:
                ...
                # Update progress bar (50% for download, 50% for conversion)
                download_progress = percent * 0.5  # Download takes 50% of total progress
                ...
                try:
                    ...
                    self.update_progress(percent * 0.5)
                    self.update_status(f"Downloading... {percent:.1f}%")
                except:
                    self.update_status("Downloading...")
                    
        elif d['status'] == 'finished':
            self.update_progress(50)  # Download complete, conversion starts
            self.update_status("Download completed, starting conversion...")
    ...
    def download_youtube_video(self, url, output_path, quality, format_choice):
        ...
        try:
            ...
            if format_choice == "mp3" and not self.check_ffmpeg():
                self.log("❌ FFmpeg not found! Please install FFmpeg for MP3 conversion.")
                messagebox.showerror("FFmpeg Required", 
                    ...
                    "or install it via: pip install ffmpeg-python")
                return
            
            # Configure yt-dlp options based on format choice
            if format_choice == "mp3":
                # For MP3, download best audio and convert manually
                ydl_opts = {
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                # For MP3 format, we need to convert the downloaded file
                if format_choice == "mp3":
                    self.update_status("Converting to MP3...")
                    self.update_progress(50)  # Download done, now converting
                    
                    # Find the downloaded file
                    downloaded_files = []
                    for ext in ['.webm', '.m4a', '.mp4', '.mkv', '.opus']:
                        potential_file = os.path.join(output_path, f"{clean_title}{ext}")
                        if os.path.exists(potential_file):
                            downloaded_files.append(potential_file)
                    
                    if not downloaded_files:
                        # Try to find any file with the title
                        for file in os.listdir(output_path):
                            if clean_title.lower() in file.lower() and not file.endswith('.mp3'):
                                downloaded_files.append(os.path.join(output_path, file))
                    
                    if downloaded_files:
                        input_file = downloaded_files[0]
                        output_file = os.path.join(output_path, f"{clean_title}.mp3")
                        
                        self.log(f"Converting: {os.path.basename(input_file)} → {os.path.basename(output_file)}")
                        
                        # Convert to MP3
                        if self.convert_to_mp3(input_file, output_file, quality):
                            # Remove original file after successful conversion
                            try:
                                os.remove(input_file)
                                self.log(f"✓ Removed temporary file: {os.path.basename(input_file)}")
                            except Exception as e:
                                self.log(f"⚠ Could not remove temporary file: {e}")
                        ...
                        raise Exception("Could not find downloaded file for conversion")
